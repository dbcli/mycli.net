...
## [Environment Variables]({filename}/pages/environment.md)<a name="environment"></a>

In addition to the config file, mycli can also be governed by environment variables.

[Read More...]({filename}/pages/environment.md)
...
Frequently used queries can be saved as named favorite queries.
...
Use the `/edit` command in the prompt to launch the default editor for a query.
...
By default, mycli doesn't expect a semi-colon at the end of a line to execute the query, but this can be changed by setting the multi-line mode.
...
## [Working with multiple servers]({filename}/pages/credentials.md)<a name="credentials"></a>

When working with multiple servers, it is possible to store their connection parameters using
DSN aliases, the system keyring, or `mysql_config_editor`.

[Read more...]({filename}/pages/credentials.md)
